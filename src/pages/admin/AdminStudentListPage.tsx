...
import {
  ...
  UserCheck, UserX, Link2, Link2Off
} from 'lucide-react';
...
type FilterStatus = 'all' | 'active' | 'inactive' | 'no-parent';
...
function StudentEditModal({ isOpen, onClose, onSave, student, classes }: {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: { name?: string; phone?: string; classId?: string; parentId?: string; isActive?: boolean }) => Promise<void>;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onSave({ name: name.trim(), phone: phone.trim() || undefined, classId: classId || undefined, isActive });
      onClose();
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b sticky top-0 bg-white">
          <h3 className="text-lg font-bold text-[#0F3D5C]">Cập nhật học sinh</h3>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[#374151] mb-1">Lớp học</label>
            <select
              value={classId}
            ...
            >
              ...
              {classes.map((cls) => (
                <option key={cls.id} value={cls.id}>{cls.name} - Khối {cls.gradeLevel}</option>
              ))}
            </select>
          </div>
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-[#374151]">Đang học (hoạt động)</span>
          </label>
...
export function AdminStudentListPage() {
  ...
  const fetchStudents = useCallback(async () => {
    setLoading(true);
    try {
      const params: Parameters<typeof api.getStudents>[0] = {
      ...
      showToast('error', 'Không thể tải danh sách học sinh');
    } finally {
      setLoading(false);
    }
  }, [pagination.page, search, gradeFilter, classFilter, statusFilter, showToast]);
  ...
  const handleSaveStudent = async (data: { name?: string; phone?: string; classId?: string; parentId?: string; isActive?: boolean }) => {
    if (!editingStudent?.user_id && !editingStudent?.id) return;
    ...
    if (res.success) {
      ...
      showToast('error', res.message || 'Không thể cập nhật học sinh');
    }
  };
  ...
  // Calculate students with no parent linkage
  const studentsWithNoParent = students.filter((s) => !s.parentId && !s.parent_name).length;
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-sm text-[#6B7280] mb-1">
            ...
            <span className="text-[#0F3D5C]">Quản lý Học sinh</span>
          </div>
          <h1 className="text-2xl font-bold text-[#0F3D5C]">Quản lý Học sinh</h1>
          <p className="text-sm text-[#6B7280] mt-1">
            {pagination.total} học sinh • {activeStudents} đang học • {studentsWithNoParent} chưa liên kết phụ huynh
          </p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
          ...
          {showFilters && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4 border-t">
              <div>
                <label className="block text-xs font-medium text-[#6B7280] mb-1">Khối lớp</label>
                <select
                  value={gradeFilter ?? ''}
                  onChange={(e) => { setGradeFilter(e.target.value ? Number(e.target.value) : null); setPagination((p) => ({ ...p, page: 1 })); }}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1C6FA8]"
                >
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] mb-1">Lớp học</label>
                <select
                  value={classFilter ?? ''}
                  onChange={(e) => { setClassFilter(e.target.value || null); setPagination((p) => ({ ...p, page: 1 })); }}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1C6FA8]"
                >
              ...
              <div>
                ...
                >
                  ...
                  <option value="no-parent">Chưa liên kết phụ huynh</option>
                </select>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#374151] uppercase tracking-wider">Học sinh</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#374151] uppercase tracking-wider">Lớp</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#374151] uppercase tracking-wider">Phụ huynh</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {students.map((student) => {
                      ...
                      return (
                        <tr key={student.user_id || student.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => handleViewStudent(student)}>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="font-medium text-[#0F3D5C]">{student.name || 'Chưa có tên'}</div>
                                <div className="text-xs text-[#6B7280]">{student.code || student.user_id}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            ...
                            {student.gradeLevel && (
                              <div className="text-xs text-[#6B7280]">Khối {student.gradeLevel}</div>
                            )}
                          </td>
                          <td className="px-4 py-3">
                            {hasParent ? (
                              <div className="flex items-center gap-1 text-sm text-green-600">
                                ...
                                <span>{student.parent_name || 'Đã liên kết'}</span>
                              </div>
                            ) : (
