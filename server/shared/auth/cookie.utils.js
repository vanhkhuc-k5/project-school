...
export function setRefreshTokenCookie(res, token) {
  const isSecured = config.IS_PRODUCTION || config.IS_STAGING;
  const cookieOptions = {
    httpOnly: true,
    secure: isSecured,
    sameSite: 'lax',
  ...
  if (typeof res.cookie === 'function') {
    ...
    const cookieStr = `${REFRESH_COOKIE_NAME}=${encodeURIComponent(token)}; Path=${REFRESH_COOKIE_PATH}; Max-Age=${Math.floor(REFRESH_COOKIE_MAX_AGE_MS / 1000)}; HttpOnly; SameSite=Lax${isSecured ? '; Secure' : ''}`;
...
export function clearRefreshTokenCookie(res) {
  ...
  if (typeof res.clearCookie === 'function') {
    ...
    const cookieStr = `${REFRESH_COOKIE_NAME}=; Path=${REFRESH_COOKIE_PATH}; Expires=Thu, 01 Jan 1970 00:00:00 GMT; HttpOnly; SameSite=Lax${isSecured ? '; Secure' : ''}`;
    res.setHeader('Set-Cookie', cookieStr);
  }
}
